...
InfJump.MouseButton1Down:connect(function()
	...
	-- for mobile jump button
	local success, err = pcall(function()
		local mobileJump = player.PlayerGui.GUI.Mobile.Phone.Jump
		if mobileJump then
			mobileJump.MouseButton1Down:Connect(function()
				if InfiniteJumpEnabled and player.Character then
					local hrp = player.Character:FindFirstChild("HumanoidRootPart")
					if hrp then
						local platform = Instance.new("Part")
...
InfAmmo.MouseButton1Down:connect(function()
	...
	while wait() do
		for i, weaponFolder in pairs(replicationstorage.Weapons:GetChildren()) do
			if weaponFolder:IsA("Folder") then
				local success, err = pcall(function()
					local ammo = weaponFolder:FindFirstChild("Ammo")
					if ammo then
						ammo.Value = 999
					end
				end)
				...
				if not success then
					print("error on " .. weaponFolder.Name .. ": " .. err)
				end
			end
		end
	end
...
RunService.RenderStepped:Connect(function()
	for i, v in pairs(Players:GetChildren()) do
		...
		if v.Character and v.Character:FindFirstChild("Head") and not v.Character.Head:FindFirstChild("esp") then
	...
	player.CharacterAdded:Connect(function(character)
		...
		if not character.Head:FindFirstChild("esp") then
			if player ~= game.Players.LocalPlayer then
				...
				nameClone:FindFirstChild("name").Text = character.Name
